...
class RecordIntakeSubmissionSignal
{
    ...
    private function payload(IntakeSubmission $submission): array
    {
        return [
            ...
            'intake_form_purpose' => $submission->form?->purpose(),
            'intake_form_language' => $submission->form?->language(),
            'intake_form_scope' => [
                'type' => $submission->form?->scopeType(),
                'client_id' => $submission->form?->scopeClientId(),
                'service_id' => $submission->form?->scopeServiceId(),
                'campaign_key' => $submission->form?->campaignKey(),
            ],
            'intake_routing_mode' => $submission->form?->routingMode(),
            'form_snapshot' => $submission->raw_payload['form_snapshot'] ?? null,
