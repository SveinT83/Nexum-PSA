...
@section('content')
    ...
    <div class="card mb-3">
        <div class="card-header d-flex align-items-center justify-content-between gap-2">
            <span class="fw-semibold">WordPress Content Context</span>
            <span class="badge text-bg-light border">{{ $campaign->contentSources->count() }} items</span>
        </div>
        <div class="card-body">
            @can('marketing.campaign.edit')
                <form method="POST" action="{{ route('tech.marketing.campaigns.content-sources.wordpress', $campaign) }}" class="row g-2 align-items-end mb-3">
                    @csrf
                    <div class="col-lg-8">
                        <label for="wordpress_url" class="form-label">WordPress URL</label>
                        <input type="url" id="wordpress_url" name="wordpress_url" class="form-control form-control-sm @error('wordpress_url') is-invalid @enderror" value="{{ old('wordpress_url') }}" placeholder="https://example.com">
                        @error('wordpress_url')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-lg-2">
                        <label for="limit" class="form-label">Posts</label>
                        <input type="number" min="1" max="10" id="limit" name="limit" class="form-control form-control-sm @error('limit') is-invalid @enderror" value="{{ old('limit', 5) }}">
            ...
            @if($campaign->contentSources->isNotEmpty())
                <div class="list-group list-group-flush border rounded">
                    @foreach($campaign->contentSources as $source)
                        <a href="{{ $source->source_url }}" target="_blank" rel="noopener" class="list-group-item list-group-item-action">
                            <div class="d-flex align-items-start justify-content-between gap-2">
                                <div>
                                    <div class="fw-semibold">{{ $source->title ?: $source->source_url }}</div>
                                    <div class="small text-muted">{{ $source->excerpt ?: parse_url($source->source_url, PHP_URL_HOST) }}</div>
                                </div>
                                <span class="badge text-bg-light border">{{ $source->published_at?->format('Y-m-d') ?? $source->fetched_at?->format('Y-m-d') }}</span>
                            </div>
                        </a>
                    @endforeach
                </div>
            @else
                <div class="text-muted small">No WordPress content is cached for this campaign.</div>
            @endif
        </div>
    </div>
                            ...
                            <form id="campaign-email-create" method="POST" action="{{ route('tech.marketing.campaigns.emails.store', $campaign) }}" data-email-create-form>
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-lg-3">
                                        ...
                                        <div class="form-text">Added after this email's normal send rhythm.</div>
    ...
    <div class="accordion mb-3" id="campaignEmailAccordion">
        @forelse($campaign->emails as $email)
            @php
                $sentCount = (int) ($email->sent_recipients_count ?? 0);
                $openCount = (int) ($email->open_events_count ?? 0);
                $clickCount = (int) ($email->click_events_count ?? 0);
            @endphp
                    ...
                    <button class="btn btn-link text-start text-decoration-none w-100 p-3" type="button" data-bs-toggle="collapse" data-bs-target="#campaignEmailPanel-{{ $email->id }}" aria-expanded="false" aria-controls="campaignEmailPanel-{{ $email->id }}">
                        <div class="d-flex align-items-center justify-content-between gap-3">
                            ...
                            <div class="d-flex align-items-center gap-2 flex-wrap justify-content-end">
                                ...
                                <span class="badge text-bg-light border text-nowrap" title="Recipients successfully sent this campaign email.">
                                    <i class="bi bi-send-check me-1" aria-hidden="true"></i>{{ $sentCount }} sent
                                </span>
                                <span class="badge text-bg-light border text-nowrap" title="Tracked open events. Email clients can block this, so it is not guaranteed read time.">
                                    <i class="bi bi-envelope-open me-1" aria-hidden="true"></i>{{ $openCount }} opened
                                </span>
                                <span class="badge text-bg-light border text-nowrap" title="Tracked link click events for this campaign email.">
                                    <i class="bi bi-cursor me-1" aria-hidden="true"></i>{{ $clickCount }} {{ $clickCount === 1 ? 'click' : 'clicks' }}
                                </span>
