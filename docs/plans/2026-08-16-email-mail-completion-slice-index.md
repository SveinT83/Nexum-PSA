...
| 6 | `HR-2026-09-01-001` | [Simple account-owned IMAP/SMTP setup](../feature-slices/2026-09-01-email-account-connection-setup.md) | Implemented On Dev / Human Review In Progress | Email Accounts is the only password-based mailbox administration surface. Dev has one active shared account with account-owned settings, separate successful IMAP/SMTP health, no provider binding, no provider credential ciphertext, no normal provider lifecycle routes, and no Email Providers card under Integrations. The superseded provider-first slice and `HR-2026-08-16-006` are historical records only. Controlled receive/send/Sent reconciliation and blank-password preservation now pass; deliberate wrong-password correction, permission, narrow-layout, and production checks continue under this replacement review. |
...
| 8 | `HR-2026-08-16-008` | [Private live invalidation transport and polling fallback](../feature-slices/2026-08-16-email-mail-private-live-invalidation-polling-fallback.md) | Implemented On Dev / Runtime Disabled / Human Review Pending | Coordinated account/user/role authority generations, resumable 100-row access recompute, scheduled start/expiry invalidation, null-safe restored writer guards, minute maintenance recovery, and a dedicated bounded current-page projection are implemented. Focused authority plus delegation/break-glass coverage passes 19 / 225. Migration `2026_09_01_100000` ran on Dev in batch 22 and all runtime flags remain false; supervised Reverb/Apache/worker/scheduler/provider/browser review and production testing remain pending. |
| 9 | `HR-2026-08-16-009` | [Reading/typing presence, shared draft locks, and stale-composer protection](../feature-slices/2026-08-19-email-mail-presence-shared-draft-locks-stale-composer.md) | Backend And Accessible UI Implemented / Runtime Disabled / Human Review Pending | The Mail composer now explicitly shares one draft, presents current holder/expiry/read-only state, fences save/attachment/discard/send, releases the lease, and permits takeover only after release/expiry. Ticket-selected replies reuse the same shared draft for an authorized peer. Focused Order 9 coverage passes 10 / 134 and Ticket integration passes 7 / 50. Flags remain false; Redis/Reverb and desktop/mobile two-user browser review remain open. |
| 10 | `HR-2026-08-16-010` | [Deterministic rules API completion](../feature-slices/2026-08-16-email-mail-deterministic-rules-api-completion.md) | Implemented On Dev / Human Review Pending | Separate durable drafts, checksum/diff publication, immutable versions, bounded local-only previews, durable run/item/action evidence, cancel/retry/full-rerun without successful-action replay, dedicated permissions/API abilities and the `email-rules` worker boundary are implemented. Migration `2026_09_03_090000` ran in Dev batch 24 without starting a run or provider operation. Named review and deployment worker verification remain open. |
...
| 12 | `HR-2026-08-16-012` | [Conversation acknowledgement and explicit multi-account actions](../feature-slices/2026-08-16-email-mail-conversation-acknowledgement-multi-account-actions.md) | Implemented On Dev / Runtime Disabled / Human Review Pending | Mail has explicit active-conversation preview/confirmation and separate personal/provider effects. API additionally accepts exact selected placements across authorized accounts and exposes content-free status/apply/retry/cancel. Bounded continuation uses the default queue and reuses the durable run/item/remote-operation ledgers. Focused coverage passes 13 / 133; the flag remains false and no shared-Dev run was created. |
...
| 16 | `HR-2026-08-16-016` | [Email/Ticket Compose And Sent Reconciliation](../feature-slices/2026-08-16-email-ticket-compose-sent-reconciliation.md) | Implemented On Dev / Runtime And Human Review Pending | The exact selected relationship now prepares one Email-owned draft, freezes Ticket/Mail/provider context, uses one outbound submission, truthfully projects accepted/unresolved/Sent once, and suppresses the legacy Ticket SMTP path. Focused coverage passes 7 / 50 and adjacent Email submission coverage passes 5 / 81. Order 9 shared UI is implemented behind its runtime flags; controlled Redis/Reverb and browser review remains gated. |
...
| 27 | `HR-2026-08-16-027` | [Microsoft 365 account driver](../feature-slices/2026-08-16-email-mail-microsoft-365-provider-driver.md) | Queued / Dependency Gated | Email-owned account connection with Integration-governed OAuth client/token lifecycle, least scope, immutable Graph IDs, delta/webhook recovery, capability mapping, migration/rollback and driver tests. |
| 28 | `HR-2026-08-16-028` | [Google Workspace account driver](../feature-slices/2026-08-16-email-mail-google-workspace-provider-driver.md) | Queued / Dependency Gated | Email-owned account connection with Integration-governed OAuth client/token lifecycle, native message/history/label semantics, authenticated Pub/Sub hints, least scope, migration/rollback and driver tests. |
...
- Password-based IMAP/SMTP endpoints, usernames, encrypted passwords, health, and runtime are owned
  by the Email account. Integrations owns no second mailbox/provider lifecycle.
- Provider folders, placement, flags, read state, drafts, Sent, Trash, and message existence remain
  authoritative on the external mail server. Here `provider` describes that remote server contract,
  never a separate Integrations-owned credential record. Nexum's `unread for me`, opened receipts,
  rules, PSA links, and governed suggestions remain separate Nexum state.
