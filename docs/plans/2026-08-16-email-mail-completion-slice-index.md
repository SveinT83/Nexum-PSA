...
| 16 | `HR-2026-08-16-016` | [Email/Ticket Compose And Sent Reconciliation](../feature-slices/2026-08-16-email-ticket-compose-sent-reconciliation.md) | Core Implemented On Dev / Shared Collaboration UI Gated / Human Review Pending | The exact selected relationship now prepares one Email-owned draft, freezes Ticket/Mail/provider context, uses one outbound submission, truthfully projects accepted/unresolved/Sent once, and suppresses the legacy Ticket SMTP path. Focused coverage passes 6 / 44 and adjacent Email submission coverage passes 5 / 81. Order 9 two-user shared UI remains separately gated. |
