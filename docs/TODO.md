...
Last reconciled against authoritative Dev on 2026-08-31.
...
| Email Provider Administration And Critical Ticket Delivery Safety | Ordinary Provider UI Superseded / Delivery Safety Human Review Pending | Svein / Codex | The ordinary provider and migration UI is superseded by the Simple Email Account Connection Setup below; the transport and delivery safety backend remains. Provider and mailbox ownership was previously explained in both admin surfaces. Safe verification failures no longer return an opaque server error. A blocked legacy account can bind to a separately created, active and exactly verified provider through a paused/drained, rollback-capable local cutover. Ticket reply SMTP uses a pre-send preparation phase, one durable reservation and no blind replay after an ambiguous provider outcome. Ticket status now distinguishes reserved, SMTP accepted and unresolved, while Mail health scopes failed jobs to `default`/`email`. Inbound evidence conflicts no longer guess a Ticket and are resolved through the audited admin queue. Current Dev has one active, exactly verified provider and one active shared Email account bound at version 1. The 2026-09-01 Full Test persisted `OK` after authenticated IMAP in 173.9 ms and SMTP in 209.0 ms. The binding actor has an explicit live-authority-coordinated View/Organize/Send grant. One self-addressed internal E2E message was accepted by SMTP, imported into INBOX, produced no Ticket while ticket ingress remained disabled, was deduplicating-appended to the provider Sent folder, imported, and reached `reconciled`. Five provider folders synchronized with no account error; the separate every-minute Nexum database worker/poll crons are present and the `email`/`default` queues are empty. The only failed-job row is an unrelated 2026-08-26 Customer Portal invitation. PHP-FPM Verify queues an opaque-ID job to the bounded CLI Email worker when its hard signal deadline is unavailable. Browser/responsive review, deliberate Ticket-ingress behavior, production rollout, and the remaining security/lifecycle checks stay pending under `HR-2026-08-16-006`. |
...
| Simple Email Account Connection Setup | Implemented On Dev / Production Human Review Pending | Codex | Approved RFC amendment `docs/rfc/2026-07-04-mail-module-full-email-client.md`, accepted ADR `docs/adr/2026-09-01-email-account-owned-imap-smtp-configuration.md`, and active Feature Slice `docs/feature-slices/2026-09-01-email-account-connection-setup.md` replace provider-first and legacy-migration administration with one Email account Add/Edit/Save-and-test flow. Existing provider rows remain internal audit evidence; ordinary administrators re-enter settings on the same account. Implemented and verified with 174 focused Mail/Integration tests and 1,436 assertions. Production browser and controlled real-account review remain under `HR-2026-09-01-001`. |
