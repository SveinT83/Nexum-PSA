...
| Nexum Integration Hub And Private MCP | In Review | Svein / Codex | Approved RFC `docs/rfc/2026-08-14-nexum-integration-hub-mcp.md`, four ADRs, and eight Feature Slices implement the first deny-by-default read-only backend: versioned capabilities/results, separate service identity and one-time grants, durable Execution/Approval/audit, emergency controls, scoped identity/Clients/Sites/domains/Integrations, and an explicitly bound Plesk read adapter. API/OpenAPI, migration, identity/key rotation, emergency, Plesk and Knowledge docs are ready. Focused Dev verification passes 34 tests / 290 assertions and affected Integration/AI regressions pass 70 / 667. The separate private MCP service consumes these contracts without credential forwarding or generic commands. Human review `HR-2026-08-15-001`, including one safe non-production Plesk read, remains pending; production/provider rollout and every mutation remain disabled. |
...
| Composer Dependency Security Advisories | In Review |  | GitHub #221 tracks the approved isolated hardening work. `composer audit --locked` on Dev reported 53 advisories across 15 locked packages on 2026-08-15; `composer validate --strict` also flags the inherited wildcard constraints for `erag/laravel-pwa` and `laravel/fortify`. Classify runtime versus development exposure, update in narrow supported batches, run the complete test/build/audit suite, and document any residual risk. Do not fold a broad framework/package upgrade into an unrelated feature delivery. |
