...
| Web Push And Inbound Email Alerts | In Progress | Svein / Codex | Approved Level 3 RFC `docs/rfc/2026-07-23-web-push-inbound-email-alerts.md` and accepted ADR `docs/adr/2026-07-23-notification-owned-web-push-channel.md`. GitHub Discussion #169 is In progress. Feature Slice 1 is implemented and migrated on Dev; persistent queue-worker verification and human browser review `HR-2026-07-24-001` remain before it can be called Done. Inbound event delivery and source read-sync remain ordered later slices. |
...
| Dev Queue And Scheduler Runtime | Blocked | Operations | No persistent `queue:work`/Horizon process or external once-per-minute `schedule:run` entry was visible from the Dev host on 2026-07-24. Configure and verify the authoritative Dev process manager before testing queued Web Push or declaring Email/notification scheduling healthy. |
