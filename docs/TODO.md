...
| --- | --- | --- | --- |
| BookStack Cross-Process Rate-Limit Coordination (#196) | Done On Dev / Live Provider Verification Pending | Svein / Codex | The former 350 ms process-local pacing let HTTP, scheduler, and queue processes independently approach BookStack's shared 180-request/minute limit. Dev now reserves one cache-backed request slot per hashed BookStack connection, defaults to one request/second, shares provider cooldown across processes, honors `Retry-After` and `X-RateLimit-Reset`, uses 15/30/60-second fallback backoff, and timestamps newly recorded errors in Admin/API status. Focused verification passes 25 tests / 186 assertions. Dev has no configured BookStack integration, so Issue #196 and `HR-2026-08-25-003` remain open until one controlled live pull with 40+ pages completes without 429 and the error timestamp is checked. |
...
| AI Model Usage And Cost Telemetry | Done On Dev | Svein / Codex | Slices 1-3 (Ledger, Call-path migration, Rate Cards) and basic Slice 4 (Admin UI) are implemented. All AI calls in Lead Intelligence and Nextcloud are migrated to the telemetry contract. Decimal-precision cost calculation via versioned rate cards is active. Human review updated to `HR-2026-08-25-003`. |
