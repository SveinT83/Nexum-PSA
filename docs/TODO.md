...
| --- | --- | --- | --- |
| Default Permission And Role Seeder Reconciliation | Ready | Svein / Codex | The 2026-08-21 Mail provider deployment repair proved that a full `RoleSeeder` sync would remove 17 current Admin and 4 current Tech grants owned by Calendar/Sales defaults. The Mail hotfix is safely additive and must not be replaced with that sync. Reconcile the authoritative permission catalog and module-owned default-grant lifecycle in a separately reviewed permission-maintenance slice before the full role seeder is used on an existing installation. |
| Email Pre-Existing Unreferenced Private-Storage Inventory | Ready | Operations / Svein | The post-recovery read-only `email:inventory-private-storage` run inspected 969 Dev files without mutation: `sent_pending` 322 (0 referenced / 322 unreferenced), `raw` 547 (462 / 85), and `attachments` 100 (30 / 70), for 492 referenced and 477 unreferenced files. It reports 28 missing `message_raw` references, 79 non-private `0644` files, 15 duplicate unreferenced checksum+size groups, and zero unsafe or unreadable files. Duplicate groups and unreferenced status do not authorize deletion; investigate send reconciliation, missing raw evidence, legacy sources/account-2 copies, provenance, retention/Ticket/legal-hold/backup relevance, and age before any separately approved mutation. No file was deleted during recovery or inventory. |
| Mail Scheduled All-Account Polling And Folder-State Recovery | Ready | Svein / Codex | Complete the immediate operational gate in `docs/plans/2026-08-16-email-mail-completion-slice-index.md`: scheduled polling must cover every active eligible account under overlap-safe account/folder locks; verify the external scheduler and Email workers; resolve the observed 53 folder-state errors without using unread state as a cursor or silently replacing UID baselines; and review the exact failed job under `HR-2026-08-15-003` without blind retry/deletion. |
| Mail Integration-Owned Provider Credentials And Endpoint Security | Done / Human Review Rework Needed | Svein / Codex | Implemented `docs/feature-slices/2026-08-16-email-mail-integration-provider-credentials-endpoint-security.md`. A 2026-08-21 portal 403 proved the shared runtime had not received the approved Mail permission catalog. After Dev database recovery, forward migration `2026_08_21_100000` ran in batch 121 and repaired all eight entries and only the intended Admin/Superuser grants additively. Sanitized readback reports 167 Admin and 216 Superuser grants, every other role unchanged, and both Email accounts still `legacy` and unbound; no seeder ran. Svein then approved the current Dev private Mail answer as exact `/32` group `tronderdata_mail_dev`. The optional environment parser accepts only one canonical RFC1918 IPv4 `/32` and otherwise omits the group; focused config/policy coverage passes 36 / 161. Sanitized runtime readback confirms the single IMAP/SMTP answer matches that rule, no legacy mapping exists, and provider connection/credential/event counts remain zero. Email polling stays paused until one Superuser-controlled preview, stage, Verify, activate, pause/drain, cutover, resume, browser review, and safe retry under `HR-2026-08-16-006`; legacy-secret purge and broad Telescope deletion remain unauthorized. |
| Mail Full Provider-Originated Reconciliation | Done / Runtime Operations Pending | Svein / Codex | Implemented `docs/feature-slices/2026-08-16-email-mail-provider-originated-reconciliation.md`: bounded resumable read-only all-folder/UID/flag/import cycles, hidden-and-attested message birth, stable negative evidence, mailbox-local NOMODSEQ verification, exact move/copy/operation reconciliation, personal-state-safe confirmed moves, deferred live-Inbox automation, durable bounded notification fan-out and external-delivery evidence, intent/lease-safe cancellation, bounded summaries/recovery, optional IDLE hints, and paged scheduled all-account correctness. The final focused SQLite bundle passes 255 / 2,225, the standalone durability gate passes 34 / 468, rolling unread-schema compatibility passes 4 / 26, the existing private MariaDB guard/path/Integration matrix passes 3 / 434, and the final MariaDB `118500` contract passes 3 / 163. The complete Email Feature directory passes 621 / 6,345 after three stale fixtures were aligned with already-enforced contracts without weakening production guards. The exact 20 Order-1-through-7 migrations `100000` through `118500` ran one per step in Dev batches 98 through 117. The inbound Ticket-message repair cursor completed through ID 363 in four pages; the failed IMAP job remains preserved for review. Controlled provider/browser/scheduler/worker/queue smoke remains operator-gated; the review summary records Svein's 2026-08-19 approval, while its older detailed checklist still needs human record reconciliation. This is not a complete repository-suite claim. |
| Mail Private Live Invalidation And Polling Fallback | Rework Needed / Runtime Disabled | Svein / Codex | The accepted replacement ADR and Feature Slice remain authoritative, but a 2026-08-21 audit proved the implemented Order 8 scaffold unsafe to activate. Migration `130000` ran in recovered Dev batch 118; forward migrations `2026_08_21_110000` and `120000` ran in batches 122 and 123 and repair the stream guard, quarantine incomplete base-authority writer guards, and fail before changing guards if live mode is enabled. Disabled mode defaults fail closed, performs no invalidation write/auth/Echo subscription, the broad global broadcast-auth route is removed, module auth is active/non-system/exact-user/2FA/CSRF/throttled, and the disabled Vite build contains no Echo/Pusher client. The client can initialize only after both its build gate and the server-rendered Mail gate pass. Complete the incompatible publisher/fanout state machine, stable call-site idempotency, exact conversation IDs, every outer transaction boundary, bounded 15/120-second catch-up, retention, exact origins/CSP, loopback supervised Reverb/worker/scheduler health and named review `HR-2026-08-16-008` before enabling. Historical Order 9/12 migrations `140000` and `150000` are inert markers run separately in batches 119 and 120 with both tables absent; collaboration and acknowledgement flags default false, and corrected schemas require new forward migrations. Orders 9–15 remain rework/dependency gated. |
...
| Mail Inbound Attachment Recovery And Download | Rework Needed / Provider Recovery Blocked | Svein / Codex | Implemented `docs/feature-slices/2026-08-15-email-mail-inbound-attachment-recovery-download.md`. The restored database baseline had zero attachment rows and only counter sum 6. An exact frozen 19-ID preflight preceded apply; local raw/legacy evidence then restored 30 rows/files across 16 messages, and the idempotent rerun returned unchanged. The four remaining parts for messages `456`, `478`, and `479` could not be recovered because the fail-closed provider resolver returned `dns_answer_set_denied`; those failed calls made no database, file, or provider mutation. The 70 unreferenced attachment files and all other private-storage evidence remain preserved; no deletion occurred. Focused `EmailAttachmentAccessRecoveryTest` passes 15 / 110, while controlled browser/access review and a separately approved safe provider-resolution path remain open. Human review `HR-2026-08-15-006` is Rework Needed. |
...
| Email Private Storage Cross-User Permissions | In Progress | Operations / Svein / Codex | `EmailPrivateStorage` limits writes to normalized `email/*` paths; the read-only inventory slice is Done with 3 tests / 21 assertions and a current 969-file no-mutation Dev snapshot. The structural contract has 61 `www-data` directories at `2770` with group-rwx access/default ACLs and no symlinks, but 79 files remain non-private. The SSH project user cannot chmod the companion runtime's files. A root/operator must change only those 79 modes to `0660` without content, ownership, move, or deletion, then rerun `email:inventory-private-storage` and PHP-FPM/queue dual-runtime smoke. Investigate 28 missing raw references, 477 unreferenced files, and 15 duplicate groups separately; the inventory found zero unsafe or unreadable files and authorizes no deletion. Keep `HR-2026-08-15-003` Pending and review the exact failed job without blind retry/deletion. |
| Mail Runtime Reliability Hardening | Done | Svein / Codex | Implemented `docs/feature-slices/2026-08-15-email-mail-runtime-reliability-hardening.md`: deterministic special-folder targets; exact UID preflight without raw user-facing provider errors or consuming mutation attempts; an atomic pre-SMTP Email-log/Message-ID reservation; truthful post-acceptance follow-up warnings; fail-closed provider Sent append; exact-draft attachment authorization; verified private writes; a tokenized provider Draft APPEND reservation with five-minute stale pre-write takeover, no replay after an unresolved response, and exact re-inferred selectable/sync-enabled Drafts selection; bounded targeted Drafts refresh; and default-collapsed Mailbox operations/Mail signature right-bar cards. Controlled Dev repair cancelled operation `23`, hid stale placement `474`, projected verified Trash UID `30177` as placement `485` in folder `141`, repaired the wrong child role to `custom`, and marked draft `1` sent/provider-deleted after exact identity proof, with zero SMTP and zero MOVE writes. No provider Sent copy was fabricated because its exact Message-ID and raw snapshot were unavailable. Runtime-focused verification passes 74 tests / 613 assertions, `EmailModuleTest.php` passes 141 / 1,227, and `InboundAutomationTest.php` passes 14 / 81. The 61-directory ownership/setgid/ACL contract is good; the current read-only 969-file snapshot still reports 79 non-private files, so root/operator mode-only normalization to `0660` plus inventory and dual-runtime smoke remain under `HR-2026-08-15-003`. |
