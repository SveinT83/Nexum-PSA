...
| --- | --- | --- | --- |
| Email Pre-Existing Unreferenced Private-Storage Inventory | Ready | Operations / Svein | The implemented read-only `email:inventory-private-storage` command inspected 939 Dev files without mutation: `sent_pending` 322 (0 referenced / 322 unreferenced), `raw` 547 (465 / 82), and `attachments` 70 (34 / 36), for 499 referenced and 440 unreferenced files. It reports 28 missing `message_raw` references, 79 non-private `0644` files, and 12 duplicate unreferenced checksum+size groups. The preceding structural audit found zero symlinks, unsafe paths, or unreadable files. Duplicate groups and unreferenced status do not authorize deletion; investigate send reconciliation, missing raw evidence, legacy sources/account-2 copies, provenance, retention/Ticket/legal-hold/backup relevance, and age before any separately approved mutation. |
...
| Mail Integration-Owned Provider Credentials And Endpoint Security | Done / Human Review Pending | Svein / Codex | Implemented `docs/feature-slices/2026-08-16-email-mail-integration-provider-credentials-endpoint-security.md`: Integration owns multi-record Email provider endpoints, password credential versions, verification/activation/revocation, safe events, public/private endpoint policy, pinned verified TLS, and a bounded legacy preview/stage/verify/cutover/rollback workflow. Email accounts retain explicit `legacy|integration` source and immutable positive binding versions; provider jobs and every cross-domain SMTP/notification sender fail stale before network and never use the system mailer. Focused coverage passes 42 / 470, the stable complete Email matrix passes 491 / 5,069, Ticket passes 132 / 955, strict notifications pass 10 / 111 plus 62 / 403 adjacent, and a disposable MariaDB `112000`-`117000` contract passes 1 / 60. No shared Dev migration, provider verification, account cutover, legacy-secret purge, or broad Telescope deletion was run. `HR-2026-08-16-006` is Pending; deployment must first inventory and exactly remediate historical provider-sensitive Telescope cohorts, then stage only one backed-up account for named review. |
| Mail Full Provider-Originated Reconciliation | Done / Human Review And Dev Operations Pending | Svein / Codex | Implemented `docs/feature-slices/2026-08-16-email-mail-provider-originated-reconciliation.md`: bounded resumable read-only all-folder/UID/flag/import cycles, hidden-and-attested message birth, stable negative evidence, mailbox-local NOMODSEQ verification, exact move/copy/operation reconciliation, personal-state-safe confirmed moves, deferred live-Inbox automation, durable bounded notification fan-out and external-delivery evidence, intent/lease-safe cancellation, bounded summaries/recovery, optional IDLE hints, and paged scheduled all-account correctness. The final focused SQLite bundle passes 255 / 2,225, the standalone durability gate passes 34 / 468, rolling unread-schema compatibility passes 4 / 26, the existing private MariaDB guard/path/Integration matrix passes 3 / 434, and the final MariaDB `118500` contract passes 3 / 163. Dev/Plesk MySQL connectivity is restored; the last read-only status reported the exact 20 Order-1-through-7 migrations `100000` through `118500` Pending, and none was applied. Provider/browser/scheduler/worker/queue smoke, deployment, and named human review remain Pending under `HR-2026-08-16-007`; focused evidence is not a complete repository-suite claim. |
| Mail Private Live Invalidation And Polling Fallback | In Progress / Runtime Activation Gated | Svein / Codex | Accepted replacement ADR `docs/adr/2026-08-16-email-live-invalidation-user-stream-fanout.md` and Feature Slice `docs/feature-slices/2026-08-16-email-mail-private-live-invalidation-polling-fallback.md` select self-hosted Reverb/Echo, one permanent user channel, and durable bounded account/global-to-user fanout while user projection versions and automatic visible 15-second polling remain authoritative. The inert `130000` durable-foundation schema/config/models are in implementation and remain unexecuted; writer integration follows only after strict state-machine, retention-summary, permission/role-path generation, and cross-engine contract evidence. Hidden inbound activation, package/build, private auth, after-commit publication, targeted bounded Livewire catch-up, revocation, exact CSP/origins, Apache/systemd/scheduler/worker health, outage fallback, and Pending review `HR-2026-08-16-008` remain in scope; no package, live migration, secret, or runtime service has been changed. |
...
| Mail Inbound Attachment Recovery And Download | Done | Svein / Codex | Implemented `docs/feature-slices/2026-08-15-email-mail-inbound-attachment-recovery-download.md`. The original raw/provider phase recovered 28 rows across 16 messages. The exact legacy persister path `email/attachments/{account}/{uid}` then supplied two count-matched, policy-checked files for each of messages `4`, `5`, and `10` without a provider search. First apply recovered 6 / 6 rows/files with counters unchanged at two; the second live apply returned `existing_rows_complete`, and all six size/SHA-1 checks pass. The bounded target is now complete at 34 rows/counter 34 across all 19 messages. Legacy source files and duplicate account-2 copies remain preserved for the separate unreferenced inventory; no deletion occurred. Focused `EmailAttachmentAccessRecoveryTest` passes 15 / 110; Pint, PHP syntax, and diff checks pass. Human review `HR-2026-08-15-006` remains Pending. |
...
| Mail Decoded Subject Search Compatibility | Done | Svein / Codex | The decoded `subject_search` projection/search surfaces remain intact and the discovered MariaDB receipt-timestamp defect is repaired. Migration `121200` ran on Dev in batch 98, removed implicit `ON UPDATE CURRENT_TIMESTAMP`, and froze 490 audit candidates. Preview/apply restored the 471 evidence-supported timestamps (439 header dates, 32 conversation boundaries), left 19 unresolved candidates untouched, and recovered exactly five falsely stale Smart Inbox suggestions. The schema now reports empty `EXTRA`; receipt-repair plus adjacent regressions pass 36 / 408. Manual search, timestamp-candidate, and rollout checks remain Pending under `HR-2026-08-15-004`. |
