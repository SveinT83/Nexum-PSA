...
| Mail Full Client, Personal Mailboxes, Rules And AI | Ready | Svein / Codex | Svein approved Level 3 RFC `docs/rfc/2026-07-04-mail-module-full-email-client.md` and accepted its four 2026-08-11 Email ADRs on 2026-08-12. They define the Email-owned full client, real Admin-configured shared IMAP accounts with separate view/organize/send grants, explicit Ticket-ingress and legacy-rule isolation, server-authoritative bidirectional folder/message sync, canonical messages/mailbox placements, manual per-user read/opened-by/live presence, account-scoped Taxonomy, a non-configurable provider/attachment/search/data-lifecycle security floor, a no-refresh unified unread conversation Inbox, several isolated Email conversations per Ticket without removing the existing Ticket-number fallback, internal-only third-party portal defaults, API-first deterministic rules, and governed AI drafts/cleanup. Historical issue #121 remains closed/not-planned source material. No code or Feature Slice is active; create the first scoped Feature Slice on authoritative Dev before implementation. Automatic external replies remain separately approval-gated. |
