...
| Marketing Domain And Email Campaign Automation | In Progress | Codex | Approved RFC 2026-06-09 plus Marketing API RFC 2026-06-17; domain foundation, Email marketing defaults/templates, mailing lists, Marketing API surface, campaign approval, due sending, dashboard, tracking foundation, campaign email cards, preview/test-send, snapshot auto-fill, AI email draft assist, AI campaign plan assist, campaign-level send rhythm, recipient batch throttling, new-contact schedule policy, and multi-list campaign audiences with recipient deduplication are implemented. Remaining email-first slices: campaign lifecycle options for repeat/stop when a sequence is complete, shared WordPress-style HTML/content editor with drag/drop blocks for Marketing, Email templates, Documentation, and Knowledge surfaces, bounce/suppression hardening, richer segmentation UI, WordPress content pull as AI/content context, Google/social integrations, and Sales/Leads consumption of marketing interest for Lead Heat and classification. Do not build separate Marketing engagement/call lists. |
