...
| Mail Conversation Acknowledgement And Explicit Selected Accounts | Implemented On Dev / Runtime Disabled / Human Review Pending | Svein / Codex | Order 12 now exposes explicit Mail preview/confirmation for the active account conversation and versioned API preview/status/apply/retry/cancel for exact active or selected cross-account placements. Frozen placement/UID/epoch/provider-binding evidence, personal coalescing, separate provider Seen, 25-item queued continuation and content-free results reuse the existing safety ledgers. Focused coverage passes 13/133. Migration `2026_08_24_140000` remains deployed with empty shared-Dev ledgers; the flag stays false and `HR-2026-08-16-012` remains the activation gate. |
