<div>
    <div class="row">
                ...
                <x-card.default title="Available Terms & Legal">
                    <div class="list-group list-group-flush" style="max-height: 400px; overflow-y: auto;">
                        @foreach($this->allTerms as $term)
                            @php
                                $isFromService = in_array((string)$term->id, $this->termsFromServices);
                                $isSelected = in_array((string)$term->id, $selectedTermIds);
                            @endphp
                            <button type="button"
                                ...
                                <span>
                                    ...
                                    @if($isFromService)
                                        <span class="badge rounded-pill bg-info text-dark ms-1" title="Included from selected services">Service</span>
                                    @endif
                                    <br>
                                    @if($term->term)
        ...
        <div class="{{ $enabled === 'disabled' ? 'col-md-12' : 'col-md-6' }}">
            <x-card.default title="Selected Terms for Package">
                <div class="list-group list-group-flush" style="max-height: 400px; overflow-y: auto;">
                    @forelse($this->selectedTerms as $term)
                        ...
                        <div class="list-group-item d-flex justify-content-between align-items-center">
                            <span>
                                <strong>{{ $term->name }}</strong>
                                @if($isFromService)
                                    <span class="badge rounded-pill bg-info text-dark ms-1">Service</span>
                                @endif
                                <br>
                                @if($term->term)
