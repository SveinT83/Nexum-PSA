...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if (! is_string($permissionsTable) || ! Schema::hasTable($permissionsTable)) {
            throw new RuntimeException('The permission schema must exist before Slice 4 Ticket Rule permissions are deployed.');
        }
        ...
        if (! $actor->isSystemActor()
            ...
            throw new RuntimeException('The protected Ticket Rule automation actor Slice 4 authority could not be verified.');
        }
    }

    public function down(): void
    {
        // Forward-only: completed Workflow execution evidence may depend on
        // this actor grant, so rollback uses a reviewed forward migration.
        app(PermissionRegistrar::class)->forgetCachedPermissions();
    }
};
